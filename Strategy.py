...
class StatArbitrageStrategy(BaseStrategy):
    def __init__(self, params: dict = None):
        ...
        self.sigma_threshold = trading_params.get(
            "sigma_threshold", 2.0
        )  # 信号触发阈值
        self.sigma_threshold_u = trading_params.get(
            "sigma_threshold_u", 4.0
        )  # 信号上限阈值
        ...
        self.bollinger_threshold = trading_params.get(
            "bollinger_threshold", 1.5
        )  # 布林带阈值

        # 3. Strategy State Variables
        self.signal_history = defaultdict(
            lambda: deque(maxlen=self.max_holding_period)
        )  # 信号历史
        self.residual_deviate_revenue_temp = defaultdict(
            lambda: deque(maxlen=self.max_holding_period)
        )  # 信号历史
        ...
        self.coin_revenues_path = defaultdict(list)  # 币种收益路径
        self.residual_deviate = defaultdict(list)  # 币种收益路径
        self.residual_deviate_revenue = defaultdict(list)  # 币种收益路径
        ...
        return {
            "coin_features": {
                "price": {
                    ...
                    "window_calc": self.regression_window,
                },
                "beta": {"window": 1},
                "residual": {"window": self.lookback_period},
                "residual_std": {"window": 1, "std_rate": self.std_rate},
    ...
    def _get_market_state(self, coin: str) -> dict:
        ...
        return {
            ...
            "position_avg_price": self.timeline.coin_features["position_avg_price"][
                "data"
            ][coin][-1],
            "current_price_COIN": self.timeline.coin_features["price"]["data"][coin][
                -1
            ],
            "current_price_BTC": self.timeline.coin_features["price"]["data"]["BTC"][
                -1
            ],
    ...
    def _can_open_position(self, state: dict) -> bool:
        ...
        residual_condition = (
            self.sigma_threshold * std < abs(residual) < self.sigma_threshold_u * std
        )
        return (
            residual_condition
            and std < self.max_std
            and state["available_capital"] > 5000
        )
    ...
    def _should_close_position(self, oldest_signal, revenue, state: dict) -> bool:
        ...
        revenue_rate = revenue / (
            abs(signal_coin) * coin_price + abs(signal_BTC) * BTC_price
        )
    ...
    def _should_close_position_1(self, signal_history, state: dict) -> list[int]:
        ...
        for i, signal in enumerate(signal_history):
            ...
            if (
                revenue_rate <= self.stop_loss
                or holding_time >= self.max_holding_period - 1
            ):
    ...
    def _calculate_position_size(self, state: dict) -> float:
        ...
        return amount / (
            state["beta"] * state["current_price_COIN"] + state["current_price_BTC"]
        )

    def _generate_long_signals(
        self, coin: str, amount: float, state: dict, signals: dict
    ) -> None:
    ...
    def _calculate_revenue(self, signal_data: tuple, state: dict) -> float:
        ...
        _, signal_coin, signal_BTC, coin_price, BTC_price = signal_data
        return float(
            signal_coin * (state["current_price_COIN"] - coin_price)
            + signal_BTC * (state["current_price_BTC"] - BTC_price)
            - self.trading_fee
            * (
                abs(signal_coin) * (state["current_price_COIN"] + coin_price)
    ...
    def _update_deviation_metrics(self, coin: str, state: dict) -> None:
        ...
        self.residual_deviate[coin].append((state["timestamp"], deviation_ratio))
    ...
    def _update_potential_revenue(self, coin: str, state: dict) -> None:
        ...
        direction = (
            -1
            if state["current_price_BTC"] < state["beta"] * state["current_price_COIN"]
            else 1
        )
        ...
        if len(self.residual_deviate_revenue_temp[coin]) == self.max_holding_period:
            # Get oldest state
            t, ratio, beta, coin_price, btc_price, dir = (
                self.residual_deviate_revenue_temp[coin][0]
            )
