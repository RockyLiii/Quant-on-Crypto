...
class Strategy(qoc.PersistableMixin):
    ...
    def step(self) -> None:
        ...
        # # temp log
        # try:
        #     paras_dict = paras_rerank.iloc[0].to_dict()

        #     with open("examples/stat_arbi-usds/temp_log.txt", "a", encoding="utf-8") as f:
        #         f.write(f"Time: {now}, Y_pred: {self.y_pred}\n")
        #         f.write("Paras_rerank:\n")
        #         for key, value in paras_dict.items():
        #             f.write(f"  {key}: {value}\n")
        #         f.write("\n" + "="*50 + "\n")
        # except Exception as e:
        #     logger.warning(f"Failed to write to temp_log.txt: {e}")
    ...
    def log_stats(self) -> None:
        ...
        if self.t % 60 == 0:
            logger.warning("Logging stats at step %d: count: %d: temp: %f", step, self.count, self.y_pred)
            ...
            plt.savefig("examples/stat_arbi-usds/asset_metrics_temp.png")
            plt.close()  # 关闭图表释放内存
            ...
            plt.legend()
            plt.savefig("examples/stat_arbi-usds/temp_values_temp.png")
            plt.close()
...
def main(cfg: Config) -> None:
    ...
    if cfg.online:
        ...
        qoc.set_clock(qoc.ClockOffline("1m", start="2026-02-13", end="2026-02-22"))
        api = ApiUsdsOffline()
...
# BINANCE_USDS_BASE_URL="https://fapi.binance.com" ./.venv/bin/python examples/stat_arbi-usds/main_rdft_temp.py
