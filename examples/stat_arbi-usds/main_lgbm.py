...
class Strategy(qoc.PersistableMixin):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        self.model_trained = pickle.load(
            open("examples/stat_arbi-usds/lgbm_model_rct.pkl", "rb")
        )
    ...
    def step(self) -> None:
        ...
        # 计算权重（基于beta调整的资金分配）
        total_exposure = price_l_now * beta_s + price_s_now * beta_l
        weight_l = max(min(beta_s * price_l_now / total_exposure, 0.7), 0.3)
        weight_s = max(min(beta_l * price_s_now / total_exposure, 0.7), 0.3)

        # 计算数量
        q_l = self.bullet_size * weight_l / price_l_now
        q_s = self.bullet_size * weight_s / price_s_now
...
def main(cfg: Config) -> None:
    cherries.log_param("group_key", "stat_arbi-usds/lgbm 2026-03-13")
    # qoc.logging.init()
    api: ApiUsds
    if cfg.online:
...
# BINANCE_USDS_BASE_URL="https://fapi.binance.com" ./.venv/bin/python examples/stat_arbi-usds/main_lgbm.py
