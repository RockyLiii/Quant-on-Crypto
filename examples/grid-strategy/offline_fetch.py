...
def check_data_coverage(library, symbol_key, start_date, end_date, interval):
    ...
    if symbol_key in library.list_symbols():
        try:
            ...
            print(f"数据库中的整体时间范围: {datetime.fromtimestamp(int(db_min_time/1000000))} 到 {datetime.fromtimestamp(int(db_max_time/1000000))}")
            print(f"请求的时间范围: {start_dt} 到 {end_dt}")
            
            # 检查数据是否覆盖了请求的时间范围
            is_in_range = (db_min_time <= start_timestamp) and (db_max_time >= end_timestamp)
            
            if not is_in_range:
                print(f"⚠️ 数据库时间范围不完全覆盖请求的时间范围")
                missing_periods = []
                
                # 添加前期缺失区间
                if db_min_time > start_timestamp:
                    missing_periods.append((start_dt, datetime.fromtimestamp(db_min_time/1000)))
                    print(f"  缺少前期数据: {start_dt} 到 {datetime.fromtimestamp(db_min_time/1000)}")
                
                # 添加后期缺失区间
                if db_max_time < end_timestamp:
                    missing_periods.append((datetime.fromtimestamp(db_max_time/1000), end_dt))
                    print(f"  缺少后期数据: {datetime.fromtimestamp(db_max_time/1000)} 到 {end_dt}")
            ...
            existing_df_datetime['datetime'] = pd.to_datetime(existing_df_datetime['Close Time'], unit='us')
            existing_df_datetime = existing_df_datetime.sort_values('datetime')
            
            # 限制在请求的时间范围内
            mask = (
                (existing_df_datetime['datetime'] >= start_dt) & 
            ...
            for i in range(1, len(filtered_df)):
                ...
                if curr_dt > expected_next_dt + timedelta(seconds=1):  # 允许1秒误差
                    ...
                    print(f"  发现数据缺口: {expected_next_dt} 到 {curr_dt - interval_delta}")
...
def get_all_timestamps_in_range(library, coins, interval, start_date, end_date):
    ...
    for coin in coins:
        ...
        if symbol_key in library.list_symbols():
            ...
            # 筛选指定日期范围内的时间戳
            filtered_timestamps = [ts for ts in df.index if start_timestamp <= ts <= end_timestamp]
