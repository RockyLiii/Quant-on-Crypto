...
class Strategy:
    ...
    def step(self) -> None:
        ...
        for symbol in self.symbols:
            ...
            klines: pl.DataFrame = self.api.klines(
                symbol, "1m", end_time=now, limit=1
            )
    ...
    def log_stats(self) -> None:
        ...
        for name, asset in account.assets.items():
            ...
            self.price_history[name].append(
                self.api.ticker_price("ETHUSDT").price
            )
        ...
        if self.t % 288 == 0:
            ...
            for symbol in self.symbols:
                ...
                vwap_steps = list(range(current_step - vwap_length + 1, current_step + 1))
                
                ax3.plot(vwap_steps, list(self.vwaps[symbol]), '-', label=f"{symbol} VWAP")
                ax3.plot(vwap_steps, list(self.vwaps_deri_1[symbol]), '--', label=f"{symbol} VWAP Deri 1")
                ax3.plot(vwap_steps, list(self.vwaps_deri_2[symbol]), '-.', label=f"{symbol} VWAP Deri 2")
