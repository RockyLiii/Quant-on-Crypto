...
class ApiOffline(TradingApi):
    ...
    symbol_average_price: dict[str, float] = attrs.field(
        factory=dict
    )  # 记录每个交易对的平均持仓价格
    ...
    def create(
                    ...
                    if not df_pandas.empty:
                        ...
                        if "Close Time" in columns:
                            all_timestamps.update(
                                ts
                                for ts in df_pandas["Close Time"]
                                if start_timestamp <= ts <= end_timestamp
                            )
                        # 情况2: 索引可能是时间戳（检查索引名称或类型）
                        elif df_pandas.index.name == "Close Time" or isinstance(
                            df_pandas.index, pd.DatetimeIndex
                        ):
                            # 如果索引是DatetimeIndex，需要转换为微秒时间戳
                            if isinstance(df_pandas.index, pd.DatetimeIndex):
                                index_timestamps = (
                                    df_pandas.index.astype(int) // 1000
                                ).tolist()  # 纳秒转微秒
                            else:
                            ...
                            all_timestamps.update(
                                ts
                                for ts in index_timestamps
                                if start_timestamp <= ts <= end_timestamp
                            )
                        else:
                            print(
                                f"警告: {symbol_key} 中没有找到 'Close Time' 列或时间索引"
                            )
        ...
        if len(timestamps) > 1:
            ...
            all_one_minute = all(
                abs(interval - one_minute_micros) <= tolerance for interval in intervals
            )

            if all_one_minute:
                print("✓ 所有时间戳间隔都在预期的1分钟范围内（允许0秒误差）")
            else:
                # 计算不符合预期的间隔数量
                non_compliant = sum(
                    1
                    for interval in intervals
                    if abs(interval - one_minute_micros) > tolerance
                )
                print(
                    f"⚠️ 发现 {non_compliant}/{len(intervals)} 个时间戳间隔不符合预期的1分钟"
                )
        ...
        print(
            f"起始时间戳: {instance.now_index} ({datetime.fromtimestamp(instance.now_index / 1000000).isoformat()})"
        )
        ...
        for symbol in symbols:
            ...
            instance.monitor_account["balances"].append(
                {"asset": base_asset, "free": "0.00000000", "locked": "0.00000000"}
            )
        ...
        for col, dtype in schema.items():
            if col in pl_data.columns:
                if dtype == pl.Datetime and pl_data[col].dtype != pl.Datetime:
                    ...
                    # 其他列类型转换
                    pl_data = pl_data.with_columns(pl.col(col).cast(dtype))
        ...
        **kwargs,
        ...
        try:
                ...
                for balance in self.monitor_account["balances"]:
                    if balance["asset"] == asset:
                        ...
                        if current_amount <= 0 and asset != "USDT":
                            current_usdt_locked = float(
                                get_asset_balance("USDT", "locked")
                            )
                            current_average_price = self.symbol_average_price.get(
                                asset, 0
                            )

                            delta_usdt_locked = (
                                amount * current_average_price * lock_rate
                            )
                        ...
                        if current_amount * amount >= 0:
                            new_average_price = (
                                (
                                    current_amount
                                    * self.symbol_average_price.get(asset, 0)
                                    + amount * price
                                )
            ...
            if side == "BUY":
                ...
                if base_free >= 0:
                    if usdt_free < total_cost:
                        raise ValueError(
                            f"Insufficient {quote_asset} balance: {usdt_free} < {total_cost}"
                        )
                    ...
                    cover_quantity = (
                        quantity if remaining_quantity < 0 else short_position
                    )
                    ...
                    if remaining_quantity > 0:
                        ...
                        if usdt_free < remaining_total_cost:
                            raise ValueError(
                                f"Insufficient {quote_asset} balance for remaining buy: {usdt_free} < {remaining_total_cost}"
                            )
            ...
            # 卖出逻辑
                ...
                # 情况2: 余额不足，先卖出已有余额，再做空剩余部分
                    ...
                    if get_asset_balance(quote_asset) < required_margin:
                        raise ValueError(
                            f"Insufficient {quote_asset} for margin: {get_asset_balance(quote_asset)} < {required_margin}"
                        )
            ...
            return OrderResponseFull(
                ...
                cummulativequoteqty=quote_amount
                if "quote_amount" in locals()
                else 0.0,  # 总价值
